	...
	listen: (bot, prefix, util, hypixel) => {
		bot.on('message', (message) => {
			...
			util.isCommand(cmd, [args.join(' ')], 'player', (err) => {
				...
				if (args[0].match(/[a-z0-9]{8}-([a-z0-9]{4}-){3}[a-z0-9]{12}/)) return hypixel.getPlayer.byUuid(args[0], (err, player) => { // checks to see if the input matches a UUID format
					if (err) return util.error(err, message.channel);
